...
# ---- 全局变量：让 worker 通过 fork 共享 df（Linux 上很省内存）----
_G = {}

def _init_worker(df, out_dir, windows, spec, symbol_col, date_col, price_cols, volume_col, trading_calendar, step):
    _G["df"] = df
    _G["out_dir"] = out_dir
    _G["windows"] = tuple(windows)
    _G["spec"] = spec
    _G["symbol_col"] = symbol_col
    _G["date_col"] = date_col
    _G["price_cols"] = price_cols
    _G["volume_col"] = volume_col
    _G["trading_calendar"] = trading_calendar
    _G["step"] = step
...
def generate_images_mp(
    ...
    # 为每个 symbol 建一个 (start, end) 切片任务，避免把 group DataFrame pickle 传给子进程
    groups = df.groupby(symbol_col, sort=False).indices  # sym -> 行号数组
    tasks = []
    for sym, idxs in groups.items():
        # idxs 在排序后应当连续；用首尾做切片
        start = int(idxs[0])
        end = int(idxs[-1]) + 1
        tasks.append((sym, start, end))

    total_syms = len(tasks)

    # 工程经验：你机器 64 核，但写 PNG 属于 I/O 重，通常 16~32 workers 更稳
    if num_workers is None:
        num_workers = min(32, os.cpu_count() or 1)

    ctx = mp.get_context("fork")  # Linux 默认就是 fork；显式写出来更清楚
    with ctx.Pool(
        processes=num_workers,
        initializer=_init_worker,
        initargs=(df, out_dir, windows, spec, symbol_col, date_col, price_cols, volume_col, trading_calendar, step),
    ) as pool:
        for _ in tqdm(
            pool.imap_unordered(_process_one_symbol, tasks, chunksize=chunksize),
            total=total_syms,
            desc=f"Multiprocessing symbols ({num_workers} workers)",
            file=sys.stdout,
        ):
            pass
...
if __name__ == "__main__":
    ...
    trading_calendar=calendar,
    step=1,
    num_workers=32,   # 你 nproc=64；这里建议先 32
    chunksize=1,      # 每个任务是一只股票；先用 1 最稳
    )
