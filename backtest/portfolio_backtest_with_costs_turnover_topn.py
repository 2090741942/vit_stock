...
def filter_top_n_by_cap(
    ...
    for dt, g in merged.groupby("end_date"):
        ...
        gg["cap_rank_on_end_date"] = np.arange(1, len(gg) + 1)
        kept.append(gg)
...
def compute_portfolio_returns(
        ...
        for decile, gg in g.groupby("decile"):
            ...
            if weighting == "equal":
                ...
                if np.any(w < 0) or w.sum() <= 0:
                    continue
                w = w / w.sum()
                port_ret = np.sum(w * gg[ret_col].values)
            else:
    ...
    if out.empty:
        raise RuntimeError("No portfolio returns generated. Check merge, dates, top_n_by_cap, or decile assignment.")
                ...
                rows.append(
                    {
                        ...
                        "symbol": row["symbol"],
                        "weight": float(wi),
                        "fwd_ret_for_drift": float(row["net_fwd_ret"]),
                    }
                )
...
def compute_turnover(
            ...
            rows.append(
                {
                    "decile": int(decile),
                    "end_date": dt,
                    "next_end_date": dt_next,
                    "turnover_per_rebalance": turnover_t,
    ...
    return out.sort_values(["decile", "end_date"]).reset_index(drop=True)
...
def main():
    parser = argparse.ArgumentParser(description="Backtest with transaction costs, annualized return, turnover, and top-N by float market cap.")
    ...
    parser.add_argument("--top_n_by_cap", type=int, default=0, help="按调仓日流通市值筛前 N；<=0 表示不筛选")
    parser.add_argument("--buy_cost_rate", type=float, default=0.0000641, help="默认 0.0641‰")
    parser.add_argument("--sell_cost_rate", type=float, default=0.0005641, help="默认 0.5641‰")
    ...
    port_rets_gross = compute_portfolio_returns(
        merged=merged,
        weighting=args.weighting,
        ret_col="gross_fwd_ret",
    )
    port_rets_net = compute_portfolio_returns(
        merged=merged,
        weighting=args.weighting,
        ret_col="net_fwd_ret",
    )
    ...
    print("\n===== step 8: compute holdings and turnover =====")
    holdings = compute_holding_weights(
        merged=merged,
        weighting=args.weighting,
    )
    turnover_ts = compute_turnover(
        holdings=holdings,
        horizon_days=args.horizon_days,
    )
    ...
    print("\n===== step 9: summarize =====")
    summary_gross = summarize_deciles(
        port_rets=port_rets_gross,
        horizon_days=args.horizon_days,
        turnover_ts=turnover_ts,
    )
    summary_net = summarize_deciles(
        port_rets=port_rets_net,
        horizon_days=args.horizon_days,
        turnover_ts=turnover_ts,
    )

    merged_out = out_dir / "merged_predictions_with_returns_and_costs_topn.csv"
    holdings_out = out_dir / f"holdings_{args.weighting}.csv"
    turnover_out = out_dir / f"turnover_{args.weighting}.csv"
    port_gross_out = out_dir / f"portfolio_returns_gross_{args.weighting}.csv"
    port_net_out = out_dir / f"portfolio_returns_net_{args.weighting}.csv"
    ...
    holdings.to_csv(holdings_out, index=False, encoding="utf-8-sig")
    turnover_ts.to_csv(turnover_out, index=False, encoding="utf-8-sig")
    port_rets_gross.to_csv(port_gross_out, index=False, encoding="utf-8-sig")
    port_rets_net.to_csv(port_net_out, index=False, encoding="utf-8-sig")
    ...
    print(f"saved holdings table       -> {holdings_out}")
    print(f"saved turnover ts          -> {turnover_out}")
    print(f"saved gross portfolio ts   -> {port_gross_out}")
    print(f"saved net portfolio ts     -> {port_net_out}")
